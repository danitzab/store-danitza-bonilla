...
const usePagination = ({ itemsPerPage, data, startFrom }) => {
  ...
  let slicedData = [...data].slice((currentPage - 1) * perPage, currentPage * perPage);
  ...
  const changePage = (page, e) => {
    ...
    if (page !== currentPage) {
      setCurrentPage(page);
      slicedData = [...data].slice((page - 1) * perPage, page * perPage);
    }
  };
  ...
  const goToPrevPage = (e) => {
    ...
    if (currentPage !== 1) {
      slicedData = [...data].slice((currentPage - 2) * perPage, (currentPage - 1) * perPage);
    }
  };
  ...
  const goToNextPage = (e) => {
    ...
    if (currentPage !== pages) {
      slicedData = [...data].slice(currentPage * perPage, (currentPage + 1) * perPage);
    }
  };
